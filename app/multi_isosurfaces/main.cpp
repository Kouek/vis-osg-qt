...
struct SelectCamera
{
	...
	osg::Vec4 ReadPixel(int col, int row)
	{
		...
		cameraRenderTo->readPixels(
			0, 0,
			cameraRenderTo->s(), cameraRenderTo->t(),
			GL_RGBA, GL_UNSIGNED_BYTE);
...
public:
	...
	virtual bool handle(const osgGA::GUIEventAdapter& eAdpt, osgGA::GUIActionAdapter& aAdpt) override
	{
		...
		case osgGA::GUIEventAdapter::RELEASE:
			if (eAdpt.getButton() == 1) {
				if (renderer->GetVolumeNum() == 0) break;
...
int main(int argc, char** argv)
{
	...
	auto misf = std::make_shared<SciVis::ScalarViser::MultiIsosurfacesRenderer>();
	...
	std::string errMsg;
	{
		...
		auto volTexSmoothed = SciVis::OSGConvertor::RAWVolume::
			NormalizedFloatToTexture(volDatSmoothed, dim, log2Dim);
