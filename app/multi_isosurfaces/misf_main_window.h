...
#include <scivis/scalar_viser/multi_isosurfaces_renderer.h>
...
class MISFMainWindow : public CCusBaseDlg
{
	Q_OBJECT

private:
	std::shared_ptr<SciVis::ScalarViser::MultiIsosurfacesRenderer> renderer;
		...
		: CCusBaseDlg(parent), renderer(renderer)
	{
		...
		connect(ui.pushButton_AddIsosurf, &QPushButton::clicked, [&]() {
			...
			auto isoVal = QInputDialog::getDouble(
				this, tr("Input Isosurface Value"), tr("IsoVal"), 30., 0., 255., 2, &ok);
			if (!ok) return;

			auto color = QColorDialog::getColor(
				Qt::white, this, tr("Input Isosurface Color"), QColorDialog::ShowAlphaChannel);
			if (!color.isValid()) return;

			sortedIsosurfs.emplace_back(
				static_cast<float>(isoVal / 255.), std::array<float, 4>{
		...
		connect(ui.pushButton_EditIsosurf, &QPushButton::clicked, [&]() {
			...
			auto newCol = QColorDialog::getColor(
				qtCol, this, tr("Input Isosurface Color"), QColorDialog::ShowAlphaChannel);
...
private:
	void updateRenderer()
	{
		...
		SciVis::ScalarViser::MultiIsosurfacesRenderer::ShadingParam shadingParam;
	...
	void updateIsosurfacesList()
	{
		...
		for (auto& isosurf : sortedIsosurfs) {
			...
			img.fill(
				QColor(255.f * col[0], 255.f * col[1],
					255.f * col[2], 255.f * col[3]));
